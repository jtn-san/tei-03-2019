...
echo -e "Instalando o PHPMyAdmin no Ubuntu Server"
    #obs: na versão >= 8.0 do MySQL 
    #não utilizamos mais o usuário root
    #para se logar no PhpMyAdmin, sendo necessário criar um novo usuário
    #sudo mysql -u root -p
    #CREATE USER 'jtn'@'localhost' IDENTIFIED WITH mysql_native_password BY 'jtn';
    #GRANT ALL PRIVILEGES ON *.* TO 'jtn'@'localhost';
    #FLUSH PRIVILEGES;
    #EXIT
